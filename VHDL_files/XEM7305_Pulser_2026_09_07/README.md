...
**IP creation now scripted** in `create_project.tcl` (config captured from
the working 2025.2 build via `write_ip_tcl`): the MIG `ddr3_256_16`, both
DDR3 adapter FIFOs (`ddr3_read_fifo` 128/32 Standard, `ddr3_write_fifo`
64-bit FWFT), and the legacy `pulse_fifo` / `fifo_photon` / `pulser_ram` /
`normal_pmt_fifo` / `readout_count_fifo`. This fixes the Mac↔Windows
divergence where the IP had to be regenerated by hand. The MIG imports its
customization from the checked-in `src/ip_cfg/ddr3_256_16.prj` (copied from
the ramtester reference) via a normalized path, not the absolute path the
GUI records, so it is reproducible on any machine. Generated IP still lands
in the git-ignored `src/ip/`. **Needs a clean-checkout build to verify.**
