"""
connection.py -- shared async LabRAD connection for the PyQt5 clients.

Python-3 port of the legacy clients/connection.py (which was py2/PyQt4). A
single connection object is shared by multiple asynchronous Qt clients so they
reuse one link to the manager and get server connect/disconnect callbacks.
Unchanged in spirit; only py2 syntax (print, except-comma) was modernised.
"""
...
class connection(object):
    ...
    def followServerConnect(self, cntx, server_name):
        ...
        if server_name in self._servers.keys():
            ...
            self._servers[server_name] = yield self.cxn[server_name]
            for action in self._on_connect.get(server_name, []):
                yield action()
    ...
    def followServerDisconnect(self, cntx, server_name):
        ...
        if server_name in self._servers.keys():
            ...
            for action in self._on_disconnect.get(server_name, []):
                yield action()
