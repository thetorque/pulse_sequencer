...
name = ScriptScanner
version = 0.9
description = Experiment orchestrator: queue / schedule / scan experiment scripts (py3)
...
### END NODE INFO
"""
# ScriptScanner -- the experiment orchestrator of the ion-clock LabRAD stack.
# It discovers experiment classes (from configuration.py), exposes them over
# LabRAD, and queues / schedules / scans them through a priority scheduler that
# runs each experiment in a worker thread while reporting progress and honouring
# pause / stop. py3 port of the legacy servers/script_scanner/script_scanner.py;
# LabRAD name, setting IDs (0-36) and signal IDs are unchanged.
#
# py3 changes: print()-functions; .keys() views wrapped in list() where a LabRAD
# list is returned; the Pause-Or-Stop setting is explicitly @inlineCallbacks
...
class ScriptScanner(LabradServer, Signals):
    ...
    def load_scripts(self):
        ...
        for import_path, class_name in config.scripts:
            try:
                ...
                print('There is no class {0} in module {1}'.format(class_name, import_path))
            except SyntaxError as e:
                print('Incorrect syntax in file {0}: {1}'.format(import_path, e))
                ...
                try:
                    ...
                    print('Name is not provided for class {0} in module {1}'.format(class_name, import_path))
                else:
    ...
    def get_scheduled(self, c):
        '''
        Returns the list of currently scheduled scans with their IDs and duration
        '''
    ...
    def new_scan(self, c, scan_script_name, measure_script_name, collection, parameter_name,
        ...
        if scan_script == measure_script:
            ...
            scan_launch = scan_methods.scan_experiment_1D_measure(scan_script.cls, measure_script.cls,
                                                                  parameter, minim, maxim, steps, units)
        scan_id = self.scheduler.add_scan_to_queue(scan_launch)
        return scan_id

    @setting(13, 'New Script Schedule', script_name='s', duration='v[s]', priority='s',
             start_now='b', returns='w')
    ...
    def register_external_launch(self, c, name):
        '''
        Issues a running ID to a script that is launched externally and not through this server.
        The external script can then update its status, be paused or stopped.
        '''
    ...
    def stopServer(self):
        ...
        try:
            ...
            # wait for all deferreds to finish
