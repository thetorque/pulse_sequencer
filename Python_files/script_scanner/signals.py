"""
signals.py -- ScriptScanner LabRAD signals (py3 port; IDs unchanged).

The signal IDs, names, and type tags are identical to the legacy server so the
PyQt5 GUI (and any listener) sees the same stream: script queued/running/paused/
stopped/finished, plus queue and schedule changes.
"""
...
class Signals(LabradServer):
    ...
    # queued scripts
    on_queued_new_script = Signal(200010, "signal_on_queued_new_script", 'wsw')  # id, name, order
