"""
scheduler.py -- ScriptScanner queue / scheduler (py3 port).

Owns the priority queue of pending scans, the dict of running scans, and the
periodic (LoopingCall) scheduled scans. Launching runs each scan's `execute`
in a worker thread via deferToThread; priority handling can pause conflicting
running scans (per configuration.allowed_concurrent) while a higher-priority one
runs, then resume them.

py3 changes: .iteritems()/.itervalues() -> .items()/.values(); print -> print();
fixed the never-exercised pop_next (indexed a list where it meant its first
element). Logic and signal emissions are otherwise unchanged.
"""
...
class priority_queue(object):
    ...
    def pop_next(self):
        ...
        return self.d[next_priority[0]].pop()

    def peek_next(self):
        next_priority = [priority for priority in self.priorities if self.d[priority]]
        if not next_priority:
            raise IndexError("queue is empty")
        return self.d[next_priority[0]][0]
...
class scheduler(object):
    ...
    def running_deferred_list(self):
        return [script.defer_on_done for script in self.running.values()
    ...
    def change_period_scheduled_script(self, scheduled_ID, period):
        try:
            ...
            raise Exception("Scheduled Script with {0} ID does not exist".format(scheduled_ID))
        else:
            lc.stop()
    ...
    def unpause_on_finish(self, result):
        unpaused_defers = []
        for ident in list(self._paused_by_script):
    ...
    def stop_running(self, ident):
        '''stop a running script by id (used on server shutdown)'''
        status = self.get_running_status(ident)
