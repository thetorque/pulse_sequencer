...
def main():
    ...
    # NB: pylabrad lowercases setting names but does NOT split camelCase, and
    # the legacy unnamed settings default to their method names -- so these are
    # getcurrentmode / isrunning / currentdataset / stoprecording (no underscores).
    pf.set_mode('Normal')
    print("mode:", pf.getcurrentmode())
    pf.set_time_length(0.05 * s)      # 50 ms windows
    pf.record_data()                  # opens a dataset + starts the recording loop
    print("recording:", pf.isrunning())

    time.sleep(1.5)                   # let a few windows accumulate + log to DV
    ds = pf.currentdataset()
    print("dataset:", ds)
    ...
    pf.stoprecording()
    print("recording after stop:", pf.isrunning())
